...
export const developments = pgTable('developments', {
  ...
  project_type: text('project_type').default('bts'),
...
export const units = pgTable('units', {
  ...
  metadata: jsonb('metadata').default(sql`'{}'::jsonb`),
  unit_status: text('unit_status').default('available'),
  current_tenancy_id: uuid('current_tenancy_id'),
  unit_mode: text('unit_mode').default('sale'),
  monthly_rent: decimal('monthly_rent', { precision: 10, scale: 2 }),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
}, (table) => ({
...
export const btrTenancies = pgTable('btr_tenancies', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  unit_id: uuid('unit_id').references(() => units.id, { onDelete: 'cascade' }).notNull(),
  development_id: uuid('development_id').references(() => developments.id, { onDelete: 'cascade' }).notNull(),
  tenant_id: uuid('tenant_id').references(() => tenants.id),
  ...
  lease_start: timestamp('lease_start', { withTimezone: true }).notNull(),
  lease_end: timestamp('lease_end', { withTimezone: true }),
  move_in_date: timestamp('move_in_date', { withTimezone: true }),
  move_out_date: timestamp('move_out_date', { withTimezone: true }),
  status: text('status').default('active').notNull(),
  access_code: text('access_code'),
  monthly_rent: decimal('monthly_rent', { precision: 10, scale: 2 }),
  deposit_amount: decimal('deposit_amount', { precision: 10, scale: 2 }),
  deposit_held: boolean('deposit_held').default(false),
  notes: text('notes'),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  updated_at: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
}, (table) => ({
  unitIdx: index('btr_tenancies_unit_idx').on(table.unit_id),
  devIdx: index('btr_tenancies_dev_idx').on(table.development_id),
  statusIdx: index('btr_tenancies_status_idx').on(table.status),
}));
...
export const maintenanceRequests = pgTable('maintenance_requests', {
  ...
  unit_id: uuid('unit_id').references(() => units.id, { onDelete: 'cascade' }).notNull(),
  development_id: uuid('development_id').references(() => developments.id, { onDelete: 'cascade' }).notNull(),
  tenancy_id: uuid('tenancy_id').references(() => btrTenancies.id),
  category: text('category').notNull(),
  ...
  assigned_vendor: text('assigned_vendor'),
  scheduled_date: timestamp('scheduled_date', { withTimezone: true }),
  scheduled_time_slot: text('scheduled_time_slot'),
  resolved_at: timestamp('resolved_at', { withTimezone: true }),
  resolution_notes: text('resolution_notes'),
  resolution_cost: decimal('resolution_cost', { precision: 10, scale: 2 }),
  tenant_rating: integer('tenant_rating'),
  tenant_feedback: text('tenant_feedback'),
  ai_diagnosis: text('ai_diagnosis'),
  ai_suggested_category: text('ai_suggested_category'),
  ai_suggested_priority: text('ai_suggested_priority'),
  is_warranty_claim: boolean('is_warranty_claim').default(false),
  ...
  unitIdx: index('maint_unit_idx').on(table.unit_id),
  devIdx: index('maint_dev_idx').on(table.development_id),
  statusIdx: index('maint_status_idx').on(table.status),
  priorityIdx: index('maint_priority_idx').on(table.priority),
  categoryIdx: index('maint_category_idx').on(table.category),
}));

export const complianceSchedule = pgTable('compliance_schedule', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  development_id: uuid('development_id').references(() => developments.id, { onDelete: 'cascade' }).notNull(),
  ...
  recurrence_months: integer('recurrence_months'),
  ...
  devIdx: index('compliance_dev_idx').on(table.development_id),
  dueIdx: index('compliance_due_idx').on(table.due_date),
  statusIdx: index('compliance_status_idx').on(table.status),
}));

export const btrAmenities = pgTable('btr_amenities', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  development_id: uuid('development_id').references(() => developments.id, { onDelete: 'cascade' }).notNull(),
  name: text('name').notNull(),
  type: text('type').notNull(),
  ...
  is_bookable: boolean('is_bookable').default(true),
  max_duration_hours: integer('max_duration_hours').default(2),
  max_advance_days: integer('max_advance_days').default(14),
  max_active_bookings: integer('max_active_bookings').default(2),
  requires_deposit: boolean('requires_deposit').default(false),
  deposit_amount: decimal('deposit_amount', { precision: 10, scale: 2 }),
  available_from: text('available_from').default('06:00'),
  available_until: text('available_until').default('22:00'),
  available_days: jsonb('available_days').default(sql`'[1,2,3,4,5,6,7]'::jsonb`),
  ...
  devIdx: index('amenities_dev_idx').on(table.development_id),
}));

export const amenityBookings = pgTable('amenity_bookings', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  amenity_id: uuid('amenity_id').references(() => btrAmenities.id, { onDelete: 'cascade' }).notNull(),
  tenancy_id: uuid('tenancy_id').references(() => btrTenancies.id, { onDelete: 'cascade' }).notNull(),
  development_id: uuid('development_id').references(() => developments.id, { onDelete: 'cascade' }).notNull(),
  ...
  amenityIdx: index('bookings_amenity_idx').on(table.amenity_id),
  tenancyIdx: index('bookings_tenancy_idx').on(table.tenancy_id),
  devIdx: index('bookings_dev_idx').on(table.development_id),
}));

export const welcomeSequences = pgTable('welcome_sequences', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  development_id: uuid('development_id').references(() => developments.id, { onDelete: 'cascade' }).notNull(),
  day_number: integer('day_number').default(1).notNull(),
  title: text('title').notNull(),
