...
export function CalendarSheet({ milestones }: Props) {
  const upcomingDates = milestones.filter((m) => !m.completed);

  const handleAddToGoogle = () => {
    // Generate Google Calendar URL
    const title = encodeURIComponent('Home Handover');
...
END:VCALENDAR`;
  ...
  return (
    <div>
      <h2 className="text-lg font-bold text-gray-900 mb-5">Add to Calendar</h2>
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          className="w-full flex items-center gap-4 p-3 rounded-xl bg-gray-50 active:scale-[0.98] transition-transform mt-2"
        >
