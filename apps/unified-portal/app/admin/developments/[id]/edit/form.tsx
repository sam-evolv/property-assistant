...
import { updateDevelopment } from '@/app/actions/developments';
...
export function DevelopmentEditForm({ development }: { development: Development }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await updateDevelopment(development.id, formData);
    } catch (err: any) {
      setError(err.message || 'Failed to update development');
      setLoading(false);
    }
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                defaultValue={development.system_instructions || ''}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gold-500"
              />
            </div>
