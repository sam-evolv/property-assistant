...
const COLUMNS = [
  ...
  { key: 'chase', label: '', width: '32px', editable: false },
];
...
export default function PipelineDetailPage() {
  ...
  const [chaseModal, setChaseModal] = useState<{ pipelineId: string; unitName: string; stage: string } | null>(null);
  const [chaseEmail, setChaseEmail] = useState<{ to: string; subject: string; body: string } | null>(null);
  ...
  const handleOpenChase = useCallback(async (row: PipelineRow, stage: string) => {
    const unitName = row.block ? `${row.block}-${row.unit_number || row.unit_name}` : (row.unit_number || row.unit_name || 'Unit');
    setChaseModal({ pipelineId: row.id, unitName, stage });
    setChaseLoading(true);
    try {
      const res = await fetch(`/api/developments/${developmentId}/pipeline/chase?pipelineId=${row.id}&stage=${stage}`);
      const data = await res.json();
      if (data.email) {
        setChaseEmail(data.email);
      }
    } catch (err) {
      console.error('Failed to generate chase email:', err);
    } finally {
      setChaseLoading(false);
    }
  }, [developmentId]);

  const getRowAlertStage = useCallback((row: PipelineRow): string | null => {
    if (row.trafficLights.contracts === 'red' || row.trafficLights.contracts === 'amber') return 'contracts';
    if (row.trafficLights.kitchen === 'red' || row.trafficLights.kitchen === 'amber') return 'kitchen';
    if (row.trafficLights.snag === 'red' || row.trafficLights.snag === 'amber') return 'snag';
    if (row.trafficLights.desnag === 'red' || row.trafficLights.desnag === 'amber') return 'desnag';
    return null;
  }, []);
    ...
    <div className="min-h-screen bg-white flex flex-col">
          ...
          <tbody>
            {filteredRows.map((row, idx) => (
              ...
              >
                ...
                <td className="px-0.5 py-0.5 text-center">
                  {(() => {
                    const alertStage = getRowAlertStage(row);
                    if (!alertStage) return null;
                    return (
                      <button 
                        className="p-0.5 hover:bg-red-100 rounded text-red-500" 
                        title="Send chase email"
                        onClick={() => handleOpenChase(row, alertStage)}
                      >
                        <MailIcon />
                      </button>
                    );
      ...
      {chaseModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/40" onClick={() => { setChaseModal(null); setChaseEmail(null); }} />
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 flex flex-col max-h-[80vh]">
            <div className="h-12 border-b border-gray-200 flex items-center justify-between px-4 flex-shrink-0">
              <h2 className="text-sm font-semibold text-gray-900">
                Chase Email - {chaseModal.unitName}
              </h2>
              <button
                onClick={() => { setChaseModal(null); setChaseEmail(null); }}
                className="p-1.5 hover:bg-gray-100 rounded-lg transition text-gray-500"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              ...
              {chaseLoading ? (
                <div className="text-center py-8">
                  <div
                    ...
                    style={{ borderColor: tokens.gold, borderTopColor: 'transparent' }}
                  />
                  <p className="text-xs text-gray-400 mt-2">Generating chase email...</p>
                </div>
              ) : chaseEmail ? (
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      value={chaseEmail.to || ''}
                      readOnly
                      className="w-full text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <textarea
                      value={chaseEmail.body || ''}
                      readOnly
                      rows={8}
                      className="w-full text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 resize-none"
                    />
                  </div>
                </div>
              ) : (
            ...
            {chaseEmail && (
              <div className="border-t border-gray-200 p-3 flex-shrink-0 flex justify-end gap-2">
                <button
                  onClick={() => { setChaseModal(null); setChaseEmail(null); }}
                ...
                <button
                  onClick={() => {
                    if (chaseEmail.to) {
                      window.open(`mailto:${chaseEmail.to}?subject=${encodeURIComponent(chaseEmail.subject)}&body=${encodeURIComponent(chaseEmail.body)}`, '_blank');
                    }
                    setChaseModal(null);
                    setChaseEmail(null);
                  }}
