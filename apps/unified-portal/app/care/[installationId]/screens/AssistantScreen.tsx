...
/* ── Animation Styles — VERBATIM from PurchaserChatTab ────────────────────── */
...
function Sources({ sources }: { sources?: { title: string; snippet: string }[] }) {
  ...
  return (
    <div className="mt-2">
      <button onClick={() => setOpen(!open)} className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 transition-colors">
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-white">
      ...
      {messages.length === 0 && showHome ? (
        ...
        <div className="flex-1 min-h-0 flex flex-col items-center justify-center px-4 overflow-y-auto pb-4">
          ...
          <h1 className="mt-3 text-center text-[17px] font-semibold leading-tight text-slate-900">
            Ask anything about{'\n'}your solar system
          </h1>

          {/* Subtitle — same text-[12px] as Property */}
          <p className="mt-1.5 text-center text-[12px] leading-relaxed max-w-[280px] text-slate-500">
            Quick answers for daily life: performance data, troubleshooting, warranties, and more.
          </p>

          {/* 2x2 Pill Grid — EXACT same className as Property PurchaserChatTab */}
          <div className="mt-4" />
          <div className="grid w-full max-w-[300px] grid-cols-2 gap-1.5">
            {PILLS.map((label, i) => (
              <button
                ...
                type="button"
                onClick={() => send(label)}
                className="flex items-center justify-center rounded-full px-2.5 py-2 text-[12px] font-medium transition-all duration-200 cursor-pointer touch-manipulation border border-slate-200 bg-white text-slate-800 shadow-sm hover:border-gold-500 hover:shadow-[0_0_10px_rgba(234,179,8,0.35)] active:scale-95"
              >
                {label}
              </button>
            ))}
      ...
      {/* ═══ INPUT BAR — iMessage pill, matches Property ═══ */}
      <div className="flex-shrink-0 px-4 pt-3 pb-2 bg-white border-t border-black/5">
        <div className="mx-auto flex max-w-3xl items-center gap-2">
          {messages.length > 0 && (
            <button onClick={() => { setMessages([]); setShowHome(true); }} className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full text-gray-500 hover:bg-black/5 hover:text-gray-700 transition-all duration-150 active:scale-95">
              <Home className="h-5 w-5" />
            </button>
          )}
          <div className="flex flex-1 items-center gap-2 rounded-full px-4 py-2.5 bg-black/5 shadow-[inset_0_1px_0_0_rgba(0,0,0,0.02),0_1px_3px_0_rgba(0,0,0,0.05)]">
            <input
              ...
              disabled={sending}
              className="flex-1 border-none bg-transparent text-[15px] placeholder:text-gray-400 focus:outline-none text-gray-900"
            />
            {input.trim() && (
              <button onClick={() => send()} disabled={sending} className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-gold-400 to-gold-500 text-white shadow-lg shadow-gold-500/25 transition-all duration-150 hover:shadow-gold-500/40 active:scale-95 disabled:opacity-50">
                <Send className="h-4 w-4" />
              </button>
            )}
          </div>
        </div>
        <p className="mt-2 text-center text-[10px] text-gray-400">Powered by AI · Information for reference only</p>
      </div>
    </div>
  );
}
