...
async function loadDemoHomeEnergy(unitId: string | null): Promise<Record<string, unknown> | null> {
  ...
  try {
    ...
    const metadata = getObj((data as Record<string, unknown>).metadata);
    const demoHome = metadata.demo_home;
    return demoHome && typeof demoHome === 'object' && !Array.isArray(demoHome)
      ? demoHome as Record<string, unknown>
      : null;
  } catch {
    return null;
  }
    ...
    if (isEnergyUsageQuestion(message)) {
      ...
      if (energyAnswer) {
        await persistMessageSafely({
          ...
          user_message: message,
          ai_message: energyAnswer,
          question_topic: 'demo_home_energy',
          source: 'purchaser_portal',
          latency_ms: Date.now() - startTime,
          metadata: {
            demoHomeEnergy: true,
            unitUid: effectiveUnitUid,
            projectId: userSupabaseProjectId,
          },
          request_id: requestId,
        });
