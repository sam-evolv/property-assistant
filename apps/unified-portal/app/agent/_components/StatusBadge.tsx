...
const BADGE_CONFIG: Record<BadgeStatus, { bg: string; border: string; dot: string; text: string; label: string }> = {
  ...
  scheduled:     { bg: '#EFF6FF', border: 'rgba(59,130,246,0.2)',  dot: '#3B82F6', text: '#1D4ED8', label: 'SCHEDULED'     },
...
export default function StatusBadge({ status }: StatusBadgeProps) {
  // Fallback guards against any unmapped status reaching the UI (e.g. legacy
  // or canonical values that bypass the API remap) so the badge never throws.
  const c = BADGE_CONFIG[status] ?? {
    bg: '#F5F5F5', border: 'rgba(163,163,163,0.2)', dot: '#A3A3A3',
    text: '#525252', label: String(status || 'UNKNOWN').toUpperCase(),
  };
